...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // Social login users have no password — tell them to use Google/Facebook
...
app.post('/api/auth/google', async (req, res) => {
  try {
    ...
    if (user) {
      ...
      if (!db.users[idx].google_id) { db.users[idx].google_id = googleId; writeDB(db); }
      user = db.users[idx];
    } else {
      user = { id:nextId(db.users), name:sanitize(name), email:email.toLowerCase(), phone:null, password:null, google_id:googleId, role:'customer', addresses:[], created_at:new Date().toISOString() };
      db.users.push(user); writeDB(db);
    }
    const token = jwt.sign({ id:user.id, email:user.email, role:user.role }, JWT_SECRET, { expiresIn:'30d' });
    res.json({ token, user:{ id:user.id, name:user.name, email:user.email, role:user.role } });
  } catch(e) { console.error('Google login error:', e); res.status(500).json({ error: 'Google login failed. Please try again.' }); }
});
...
app.post('/api/auth/facebook', async (req, res) => {
  try {
    ...
    if (!fbUserData.email) return res.status(400).json({ error: 'Facebook account does not have a public email. Please use email/password login.' });
    ...
    if (user) {
      ...
      user = { id:nextId(db.users), name:sanitize(name), email:email.toLowerCase(), phone:null, password:null, facebook_id:facebookId, role:'customer', addresses:[], created_at:new Date().toISOString() };
      db.users.push(user); writeDB(db);
    }
    const token = jwt.sign({ id:user.id, email:user.email, role:user.role }, JWT_SECRET, { expiresIn:'30d' });
    res.json({ token, user:{ id:user.id, name:user.name, email:user.email, role:user.role } });
  } catch(e) { console.error('Facebook login error:', e); res.status(500).json({ error: 'Facebook login failed. Please try again.' }); }
});
...
app.post('/api/auth/forgot-password', async (req, res) => {
  try {
    ...
    const db   = readDB();
    const user = db.users.find(u => u.email === input || u.email === input.toLowerCase() || (u.phone && u.phone === input.replace(/\D/g,'').slice(-10)));
    if (!user) return res.json({ message: 'If this account exists, an OTP has been sent to the registered email.' });
    if (!user.email) return res.status(400).json({ error: 'No email address linked to this account. Please contact support on WhatsApp.' });
    ...
    <div style="font-family:Arial,sans-serif;max-width:500px;margin:auto">
      ...
      <div style="background:#f5f8ff;padding:28px;border-radius:0 0 12px 12px">
        ...
        <p style="color:#e53e3e;font-size:13px">⚠️ Never share this OTP with anyone.</p>
      </div>
    </div>`;
    ...
    if (!emailResult || !emailResult.sent) {
      ...
      return res.json({ message: isNotConfigured ? 'OTP generated! Email is not configured yet — please check Render logs for the OTP, or contact admin.' : 'OTP generated but email failed to send. Please contact admin on WhatsApp.', email_masked: maskedEmail, email_sent: false, debug_otp: isNotConfigured ? otp : undefined });
    }
...
app.post('/api/auth/verify-otp', (req, res) => {
  try {
    ...
    const db   = readDB();
    const user = db.users.find(u => u.email === input || (u.phone && u.phone === input.replace(/\D/g,'').slice(-10)));
...
app.post('/api/auth/reset-password', (req, res) => {
  try {
    ...
    let decoded;
    try { decoded = jwt.verify(reset_token, JWT_SECRET); } catch(e) { return res.status(400).json({ error: 'Reset link has expired. Please start again.' }); }
...
app.post('/api/shipping/calculate', (req, res) => {
  try {
    ...
    res.json({ shipping, free: shipping === 0, subtotal, free_threshold:freeMin, amount_for_free:amountNeededForFree, default_charge:settings.default_shipping_charge||80, shipping_message:settings.shipping_message||('Free shipping on orders above Rs.'+freeMin) });
...
app.post('/api/payment/create-order', authMiddleware, (req, res) => {
  try {
    ...
    razorpay.orders.create({ amount: amountPaise, currency: 'INR', receipt: 'PPP_' + Date.now(), notes: { user_id: req.user.id } }, (err, order) => {
      if (err) { console.error('Razorpay create-order error:', JSON.stringify(err)); return res.status(500).json({ error: 'Could not create payment: ' + (err.error?.description || err.message || JSON.stringify(err)) }); }
...
app.post('/api/payment/verify', authMiddleware, async (req, res) => {
  try {
    ...
    if (!razorpay_order_id || !razorpay_payment_id) {
      return res.status(400).json({ error: 'Missing payment details. Payment ID: ' + (razorpay_payment_id || 'unknown') });
    }
    ...
    if (razorpay_signature && RAZORPAY_KEY_SECRET) {
      ...
      if (!signatureValid) {
        ...
        console.log('KEY_SECRET last 4 chars:', RAZORPAY_KEY_SECRET.slice(-4));
      }
    } else {
      console.log('⚠️  Skipping signature check — KEY_SECRET not set or signature missing');
    }

    // ── If signature failed, verify via Razorpay API ────────────
    if (!signatureValid) {
      if (!razorpay) {
        console.error('❌ Razorpay not initialised — RAZORPAY_KEY_ID / RAZORPAY_KEY_SECRET missing in Render env');
        return res.status(503).json({
          error: 'Payment gateway not configured on server. Please contact admin with your Payment ID: ' + razorpay_payment_id,
        });
      }
      try {
        const payment = await razorpay.payments.fetch(razorpay_payment_id);
        const validStatus = payment && (payment.status === 'captured' || payment.status === 'authorized');
        const validOrder  = payment && payment.order_id === razorpay_order_id;
        console.log('Razorpay API fallback — status:', payment?.status, '| order match:', validOrder);
        if (!validStatus || !validOrder) {
          return res.status(400).json({
            error: 'Payment not verified. Status: ' + (payment?.status || 'unknown') + '. Payment ID: ' + razorpay_payment_id,
          });
        }
        console.log('✅ Payment verified via Razorpay API (signature bypassed)');
      } catch (apiErr) {
        console.error('Razorpay API verify failed:', apiErr.message);
        return res.status(400).json({
          error: 'Payment verification failed. Payment ID: ' + razorpay_payment_id + '. Please WhatsApp us — we will confirm your order manually within 30 minutes.',
        });
      }
    }
    ...
    const order = {
      ...
      notes: sanitize(notes) || null, cancel_reason: null,
    ...
    console.log('✅ Order created:', order_number, '| payment:', razorpay_payment_id, '| total: Rs.', total);
...
app.post('/api/payment/recover', authMiddleware, async (req, res) => {
  try {
    ...
    if (!resolvedItems.length) return res.status(400).json({ error: 'Could not recover items. Please contact admin with payment ID: ' + razorpay_payment_id });
    const settings  = db.settings;
    const shipping  = calculateShipping(resolvedItems, db.products, settings);
    const gst       = Math.round(subtotal * 0.18 * 100) / 100;
    const total     = subtotal + gst + shipping;
    ...
    const order = {
      ...
      shipping_address: JSON.stringify(shipping_address || {}),
      payment_method: 'online', payment_status: 'paid',
      razorpay_order_id: payment.order_id, razorpay_payment_id,
      status: 'confirmed', tracking_number: null, tracking_url: null,
...
// ── IMPORTANT: /track/:order_number MUST come BEFORE /:id ──────
// If /:id is first, Express matches 'track' as the id value and
...
app.get('/api/search/suggestions', (req, res) => {
  try {
    ...
    if (q.length < 1) {
      ...
      const popular  = db.products.filter(p => p.is_active && p.is_bestseller).slice(0, 4).map(p => ({ id:p.id, name:p.name, price:p.price, image_url:p.image?BASE_URL+p.image:null, category:db.categories.find(c=>c.id===p.category_id)?.name||'' }));
      const recent   = db.products.filter(p => p.is_active).sort((a,b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 4).map(p => ({ id:p.id, name:p.name, price:p.price, image_url:p.image?BASE_URL+p.image:null, category:db.categories.find(c=>c.id===p.category_id)?.name||'' }));
      return res.json({ trending, popular, recent, products:[], categories:[] });
    }
    trackSearch(q);
    const products = db.products.filter(p => { if (!p.is_active) return false; const name=(p.name||'').toLowerCase(),sku=(p.sku||'').toLowerCase(),desc=(p.description||'').toLowerCase(); return name.includes(q)||sku.includes(q)||desc.includes(q)||q.split(' ').every(word=>name.includes(word)); }).slice(0, 6).map(p => ({ id:p.id, name:p.name, price:p.price, old_price:p.old_price, sku:p.sku, image_url:p.image?BASE_URL+p.image:null, category:db.categories.find(c=>c.id===p.category_id)?.name||'', stock:p.stock }));
...
app.get('/api/search/trending', (req, res) => {
  try {
    const db = readDB();
    const trending = (db.search_logs || []).sort((a,b) => b.count - a.count).slice(0, 10).map(s => ({ query: s.query, count: s.count }));
    res.json(trending);
  } catch(e) { res.json([]); }
});
...
app.put('/api/admin/orders/:id', adminMiddleware, (req, res) => {
  try {
    ...
    const idx = db.orders.findIndex(o => o.id === parseInt(req.params.id));
    if (idx === -1) return res.status(404).json({ error: 'Order not found' });
    const allowed = ['status','payment_status','payment_method','tracking_number','tracking_url','shipping_address','notes','cancel_reason','total','subtotal','gst'];
    allowed.forEach(field => { if (req.body[field] !== undefined) db.orders[idx][field] = req.body[field]; });
    db.orders[idx].updated_at = new Date().toISOString();
...
app.get('/api/admin/users', adminMiddleware, (req, res) => {
  try {
    ...
    const result = users.map(u => { const { password, ...safe } = u; const orderCount = db.orders.filter(o => o.user_id === u.id).length; const totalSpent = db.orders.filter(o => o.user_id===u.id && o.payment_status==='paid').reduce((s,o)=>s+o.total,0); return { ...safe, order_count: orderCount, total_spent: totalSpent }; });
...
app.put('/api/admin/users/:id', adminMiddleware, (req, res) => {
  try {
    ...
    if (req.body.new_password) { if (req.body.new_password.length < 8) return res.status(400).json({ error: 'Password must be at least 8 characters' }); db.users[idx].password = require('bcryptjs').hashSync(req.body.new_password, 12); }
    db.users[idx].updated_at = new Date().toISOString();
    writeDB(db);
...
app.get('/api/admin/enquiries', adminMiddleware, (req, res) => {
  const db = readDB();
  res.json((db.enquiries||[]).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)));
});
...
app.delete('/api/admin/enquiries/:id', adminMiddleware, (req, res) => {
  ...
  db.enquiries.splice(idx, 1); writeDB(db);
...
app.put('/api/admin/enquiries/:id/read', adminMiddleware, (req, res) => {
  ...
  const idx = (db.enquiries||[]).findIndex(e => e.id === parseInt(req.params.id));
  if (idx === -1) return res.status(404).json({ error: 'Not found' });
  db.enquiries[idx].is_read = true; writeDB(db);
  res.json({ message: 'Marked as read' });
});

app.delete('/api/admin/search-logs', adminMiddleware, (req, res) => {
  const db = readDB(); db.search_logs = []; writeDB(db);
...
app.put('/api/admin/profile', adminMiddleware, (req, res) => {
  try {
    ...
    writeDB(db);
    const { password, ...safe } = db.users[idx];
    const token = require('jsonwebtoken').sign({ id: db.users[idx].id, email: db.users[idx].email, role: 'admin' }, JWT_SECRET, { expiresIn: '30d' });
...
app.put('/api/admin/password', adminMiddleware, (req, res) => {
  try {
    ...
    if (!require('bcryptjs').compareSync(current_password, db.users[idx].password)) return res.status(401).json({ error: 'Current password is incorrect' });
