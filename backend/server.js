...
try {
  if (EMAIL_USER && EMAIL_PASS) {
    ...
    console.log('Email configured (smtp.gmail.com:587)');
...
// ── FILE UPLOAD — CHANGE 1: supports up to 6 product images ──
// NOTE: Use upload.array('images', 6) on product routes
const upload = multer({
...
async function sendSMS(phone, message) {
  ...
  if (!apiKey) {
    console.log('SMS not configured — add FAST2SMS_API_KEY to Render environment');
    return { sent: false };
  }
  const normalized = String(phone || '').replace(/\D/g, '').slice(-10);
  if (normalized.length !== 10) {
    console.log('SMS skipped — invalid phone number:', phone);
    return { sent: false };
  }
  try {
...
app.post('/api/settings/logo', adminMiddleware, uploadLogo.single('logo'), async (req, res) => {
  try {
    ...
    const db = readDB();
    if (db.settings.logo_url) await deleteFromCloudinary(db.settings.logo_url);
    let logoUrl;
    if (cloudinary) {
      logoUrl = await uploadToCloudinary(req.file.buffer, 'logo');
      if (!logoUrl) return res.status(500).json({ error: 'Cloudinary upload failed. Check CLOUDINARY credentials in Render.' });
    } else {
      const dir = path.join(__dirname, 'uploads', 'logo');
      if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
      const ext = path.extname(req.file.originalname).toLowerCase();
      fs.writeFileSync(path.join(dir, 'logo' + ext), req.file.buffer);
      logoUrl = '/uploads/logo/logo' + ext;
    }
...
// ════════════════════════════════════════════════════════════════
//   PRODUCTS — CHANGE 5: GET returns images_urls array
// ════════════════════════════════════════════════════════════════
app.get('/api/products', (req, res) => {
  try {
    ...
    list = list.map(p => {
      ...
      // CHANGE 5: added images_urls
      const image_url   = p.image  ? (p.image.startsWith('http')  ? p.image  : BASE_URL+p.image)  : null;
      const images_urls = (p.images||[]).map(img => img.startsWith('http') ? img : BASE_URL+img);
      return { ...p, category_name:db.categories.find(c=>c.id===p.category_id)?.name||'', image_url, images_urls, avg_rating:avgRating, review_count:revs.length };
    });
...
app.get('/api/products/:id', (req, res) => {
  ...
  const image_url   = p.image ? (p.image.startsWith('http') ? p.image : BASE_URL+p.image) : null;
  const images_urls = (p.images||[]).map(img => img.startsWith('http') ? img : BASE_URL+img);
  res.json({ ...p, category_name:db.categories.find(c=>c.id===p.category_id)?.name||'', image_url, images_urls, avg_rating:avgRating, review_count:revs.length, reviews:revs.slice(0,10) });
});

// CHANGE 2: POST /api/products — supports up to 6 images
app.post('/api/products', adminMiddleware, upload.array('images', 6), async (req, res) => {
  try {
    const db = readDB();
    ...
    const gstRate = req.body.gst_rate !== undefined && req.body.gst_rate !== '' ? parseFloat(req.body.gst_rate) : null;
    ...
    const prod = {
      ...
      gst_rate:gstRate,
      image: uploadedImages[0] || null,       // primary image (backward compat)
      images: uploadedImages,                  // all images array
      is_new: is_new === '1',
...
// CHANGE 3: PUT /api/products/:id — appends new images to existing (up to 6 total)
app.put('/api/products/:id', adminMiddleware, upload.array('images', 6), async (req, res) => {
  try {
    const db = readDB();
    const idx = db.products.findIndex(p => p.id === parseInt(req.params.id));
    if (idx === -1) return res.status(404).json({ error: 'Product not found' });

    let existingImages = db.products[idx].images || (db.products[idx].image ? [db.products[idx].image] : []);

    // If new files uploaded, add to existing (up to 6 total)
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        if (existingImages.length >= 6) break;
        if (cloudinary) {
          const url = await uploadToCloudinary(file.buffer, 'products');
          ...
          const filename = Date.now() + '-' + Math.round(Math.random()*1e9) + ext;
          fs.writeFileSync(path.join(dir, filename), file.buffer);
          existingImages.push('/uploads/products/' + filename);
        }
      }
    }

    const { name, description, sku, category_id, price, old_price, stock, is_new, is_bestseller, is_active } = req.body;
    const updatedShipping = req.body.shipping_charge !== undefined
      ? (req.body.shipping_charge === '' || req.body.shipping_charge === null ? null : parseFloat(req.body.shipping_charge))
      : db.products[idx].shipping_charge;
      const updatedGst = req.body.gst_rate !== undefined 
      ? (req.body.gst_rate===''||req.body.gst_rate===null?null:parseFloat(req.body.gst_rate)) : db.products[idx].gst_rate;

    db.products[idx] = {
      ...db.products[idx],
      name: sanitize(name) || db.products[idx].name,
      description: sanitize(description) ?? db.products[idx].description,
      sku: sanitize(sku) ?? db.products[idx].sku,
...
app.delete('/api/products/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    const images = db.products[idx].images || (db.products[idx].image ? [db.products[idx].image] : []);
    for (const img of images) await deleteFromCloudinary(img);
    db.products.splice(idx,1); writeDB(db);
    res.json({ message:'Product deleted' });
  } catch(e) { console.error(e); res.status(500).json({ error:'Server error' }); }
});

// Delete primary image (backward-compat)
app.delete('/api/products/:id/image', adminMiddleware, async (req, res) => {
  ...
  if (db.products[idx].image) {
    ...
    db.products[idx].images = (db.products[idx].images||[]).filter(img => img !== db.products[idx].image);
    db.products[idx].image = db.products[idx].images[0] || null;
    writeDB(db);
  }
  res.json({ message:'Image removed' });
});

// CHANGE 4: DELETE a specific product image by index
app.delete('/api/products/:id/images/:index', adminMiddleware, async (req, res) => {
  const db = readDB();
  const idx = db.products.findIndex(p => p.id === parseInt(req.params.id));
  if (idx === -1) return res.status(404).json({ error: 'Product not found' });

  const imgIndex = parseInt(req.params.index);
  const images = db.products[idx].images || [];
  if (imgIndex < 0 || imgIndex >= images.length) return res.status(400).json({ error: 'Invalid image index' });

  await deleteFromCloudinary(images[imgIndex]);
  images.splice(imgIndex, 1);
  db.products[idx].images = images;
  db.products[idx].image  = images[0] || null;
  writeDB(db);
...
// CHANGE 6: Category image upload
app.post('/api/categories/:id/image', adminMiddleware, uploadLogo.single('image'), async (req, res) => {
  try {
    ...
    if (db.categories[idx].image_url) await deleteFromCloudinary(db.categories[idx].image_url);

    let imageUrl;
    if (cloudinary) {
      imageUrl = await uploadToCloudinary(req.file.buffer, 'categories');
      if (!imageUrl) return res.status(500).json({ error: 'Cloudinary upload failed' });
    } else {
      const dir = path.join(__dirname, 'uploads', 'categories');
      if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
      const ext = path.extname(req.file.originalname).toLowerCase();
      const filename = 'cat-' + db.categories[idx].id + ext;
      fs.writeFileSync(path.join(dir, filename), req.file.buffer);
      imageUrl = '/uploads/categories/' + filename;
    }

    db.categories[idx].image_url = imageUrl;
    writeDB(db);
    res.json({ message: 'Category image uploaded', image_url: imageUrl });
  } catch(e) { console.error(e); res.status(500).json({ error: 'Upload failed: ' + e.message }); }
});

// CHANGE 6: Remove category image
app.delete('/api/categories/:id/image', adminMiddleware, async (req, res) => {
  const db = readDB();
  const idx = db.categories.findIndex(c => c.id === parseInt(req.params.id));
  if (idx === -1) return res.status(404).json({ error: 'Category not found' });
  if (db.categories[idx].image_url) {
    await deleteFromCloudinary(db.categories[idx].image_url);
    db.categories[idx].image_url = null;
    writeDB(db);
  }
  res.json({ message: 'Category image removed' });
});
...
app.post('/api/payment/verify', authMiddleware, async (req, res) => {
  try {
    ...
    let shippingAddr2 = {};
    try { shippingAddr2 = JSON.parse(order.shipping_address); } catch(e) {}
    const smsPhone2 = shippingAddr2.phone || user?.phone || '';
    if (smsPhone2) sendSMS(smsPhone2, `Dear ${user?.name}, your order ${order_number} is CONFIRMED. Payment received: Rs.${total}. Track at printersreports.in/track.html - PrintersReports`);
...
app.post('/api/payment/recover', authMiddleware, async (req, res) => {
  try {
    ...
    const settings = db.settings, shipping = calculateShipping(resolvedItems, db.products, settings);
    // Calculate GST per item using product-specific rate or global rate
    let gst = 0;
    resolvedItems.forEach(item => {
    const product = db.products.find(p => p.id === item.product_id);
    const itemGstRate = (product && product.gst_rate !== null && product.gst_rate !== undefined)
    ? product.gst_rate
    : (parseFloat(db.settings.gst_rate) || 18);
    gst += Math.round(item.price * item.qty * itemGstRate / 100 * 100) / 100;
    });
    gst = Math.round(gst * 100) / 100; total = subtotal+gst+shipping;
    const order_number = 'PPP'+Date.now(), orderId = nextId(db.orders);
...
app.post('/api/orders/cod', authMiddleware, async (req, res) => {
  try {
    ...
    let shippingAddr = {};
    try { shippingAddr = JSON.parse(order.shipping_address); } catch(e) {}
    const smsPhone = shippingAddr.phone || user?.phone || '';
    if (smsPhone) sendSMS(smsPhone, `Dear ${user?.name}, your order ${order_number} has been placed successfully. Total: Rs.${total}. Track at printersreports.in/track.html - PrintersReports`);
...
app.put('/api/orders/:id/status', adminMiddleware, async (req, res) => {
  try {
    ...
    const smsMessages = {
      confirmed:  `Dear ${user?.name}, your order ${db.orders[idx].order_number} is CONFIRMED and being prepared. - PrintersReports`,
      processing: `Dear ${user?.name}, your order ${db.orders[idx].order_number} is being PACKED. - PrintersReports`,
      shipped:    `Dear ${user?.name}, your order ${db.orders[idx].order_number} has been SHIPPED. Tracking: ${tracking_number||'will be shared soon'}. - PrintersReports`,
      delivered:  `Dear ${user?.name}, your order ${db.orders[idx].order_number} has been DELIVERED. Thank you for shopping! - PrintersReports`,
      cancelled:  `Dear ${user?.name}, your order ${db.orders[idx].order_number} has been CANCELLED. Contact us on WhatsApp for help. - PrintersReports`,
    };
    if (user?.phone && smsMessages[status]) sendSMS(user.phone, smsMessages[status]);
...
app.get('/api/admin/orders', adminMiddleware, (req, res) => {
  ...
  orders = orders.map(o => {
    ...
    const ois=(db.order_items||[]).filter(i=>i.order_id===o.id);
    const item_details = ois.map(i => {
      const p=db.products.find(p=>p.id===i.product_id);
      return { product_id:i.product_id, name:p?.name||'Product', qty:i.qty, price:i.price };
    });
    const items = item_details.map(i => `${i.name} ×${i.qty}`).join(', ');
    return { ...o, customer_name:u?.name, customer_email:u?.email, customer_phone:u?.phone, items, item_details };
  });
...
setupDatabase().then(() => {
  app.listen(PORT, () => {
    ...
    console.log('SMS:        ' + (process.env.FAST2SMS_API_KEY ? 'configured' : 'NOT configured — add FAST2SMS_API_KEY'));
